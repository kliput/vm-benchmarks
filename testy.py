...
class Tests(object):
    def __init__(self):
        ...
        z iperf -s
        '''
        self.iperf_server = 'localhost'

        print 'Setting up iperf servers...'
        try:
            self.serv1 = subprocess.Popen(['iperf','-s','-p10000', '-D'])
            self.serv2 = subprocess.Popen(['iperf','-s','-u','-p10001', '-D'])
        except subprocess.CalledProcessError:
            print 'Brak programu iperf, proba pobrania'
            subprocess.check_call(['sudo', 'apt-get', '-qq', '-y', 'install', 'iperf'])
            
        print 'Done.'

    def t_iperf(self):
        print 'test wydajności tcp'
        subprocess.check_call(['iperf','-fM','-p10000','-c',self.iperf_server])

        print 'test wydajności udp'    
        subprocess.check_call(['iperf','-u','-p10001','-fM','-c',self.iperf_server])
        
    def t_jumboframes(self):
        print 'test duzych pakietow tcp'
        subprocess.check_call(['iperf','-M9000','-p10000','-fM','-c',self.iperf_server])

        print 'test duzych pakietow udp'
        subprocess.check_call(['iperf','-M9000','-p10001','-u','-fM','-c',self.iperf_server])
    ...
    def cleanup(self):
        self.null.close()
        print 'Tearing down iperf server...'
        self.serv1.kill()
        self.serv2.kill()
